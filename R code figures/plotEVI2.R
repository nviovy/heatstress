library(ggplot2)
 cc <- read.table("histtEVI2_1",header=FALSE,sep = " ",dec=".")
 cc2 <- read.table("histtEVI2_2",header=FALSE,sep = " ",dec=".")
 cc3 <- read.table("histtEVI2_3",header=FALSE,sep = " ",dec=".")
 cc4 <- read.table("histtEVI2_4",header=FALSE,sep = " ",dec=".")
cc5 <- read.table("histtEVI2_5",header=FALSE,sep = " ",dec=".")
...
names(data)<-c("temperature","EVI2_delta","deltamin","deltamax","types")
data$temperature=as.numeric(data$temperature)
data$deltamin=as.numeric(data$deltamin)
data$deltamax=as.numeric(data$deltamax)
data$EVI2_delta=as.numeric(data$EVI2_delta)
data[data==0]<-NaN

myplot<-ggplot(data, aes(temperature, EVI2_delta, colour=types, fill=types)) +
