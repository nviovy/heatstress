...
float danandvi[NVEG][NBL][NBCTS], danats[NBL][NBCTS],danandviav[NVEG][NBL][NBCTS],danandviap[NVEG][NBL][NBCTS],dndvim[NVEG][NBL][NBCTS],dndviap[NVEG][NBL][NBCTS],datatsmax[DYEAR-PYEAR+1][NBL][NBCTS],datatstamax[NBL][NBCTS],map[NVEG][NBL][NBCTS],datatstamax[NBL][NBCTS],meanmaxts[NBL][NBCTS],tsmseuil[NVEG][NBL][NBCTS],nbvalseuil[NVEG][NBL][NBCTS],year1[NVEG][NBL][NBCTS],year2[NVEG][NBL][NBCTS],diffout[NVEG][NBL][NBCTS],anaav[NVEG][NBL][NBCTS],nbvaldiff[NVEG][NBL][NBCTS];
...
void main(int argc,char *argv[])
{
  ...
  int varanandvi[NBYEAR],varanandviav[NBYEAR],varanandviap[NBYEAR],vartsmax,vartstamax,varndvim[NBYEAR],varanats[NBYEAR],var,nvarlonts,nvarlatts,varmtmax,nxii,nyii,nh2D,nxv,nyv,vartseuil,varnbyear,varyear1,varyear2,nveg,varndviap[NBYEAR],varanaav;
  float diffvi,tsm,histom[NVEG2][NBVAL2],histts[NVEG2][NBVALTS],varm[NVEG2][NBVAL2],histom2[NVEG2][NBVAL2],histom3[NVEG2][NBVAL2];
  int nvalts[NVEG2][NBVALTS],year,i,l,c,t,v,l2;
  float nval[NVEG2][NBVAL2];
  long ncount[4],start[4];
  char nfic[100],str70[70];
  float fillval=-1e32,ats,fillval2=-1e20,tsmm,lat;
...
for (v=0;v<NVEG2;v++)
   {
     sprintf (ficout,"histtndvig2_%d",v+1);
     fout[v]=fopen(ficout,"w");
     sprintf (ficout,"histtstndvig2_%d",v+1);
 ...
 ncido=nccreate("data/repndvipftglob2.nc",NC_64BIT_OFFSET);
 ...
 for (l=0;l<NBLTS;l+=NBL)
   {
		 ...
		 if ((danandviav[v][l2][c]>-1)&&(danandviav[v][l2][c]<=1)&&(danandviap[v][l2][c]>-1)&&(danandviap[v][l2][c]<=1))
		   {
		     ...
		     if (dndviap[v][l2][c]>0.05) diffvi=(danandviap[v][l2][c]-danandviav[v][l2][c])/dndviap[v][l2][c];
		     if ((diffvi<-0.005) && (danats[l2][c]>4)&&(dndviap[v][l2][c]>0.3))
		       //if ((danats[l][c]>4))
		       {
		     ...
		     tsmm=meanmaxts[l2][c];
		     if ((tsm>40)&&(dndviap[v][l2][c]>0.3))
		       {
			 diffout[v][l2][c]+=diffvi;
			 anaav[v][l2][c]+=danandviav[v][l2][c];
			 nbvaldiff[v][l2][c]++;
		       }
		     if ((tsm>20)&&(tsm<65)&&(tsmm>20)&&(tsmm<65)&&(dndvim[v][l2][c]>0.<45)&&( meanmaxts[l2][c]<45)&&(dndviap[v][l2][c]>0.3))
		       {
		     ...
		     if ((tsm>20)&&(tsm<65)&&(dndvim[v][l2][c]>0.4)&&( meanmaxts[l2][c]<45)&&(dndviap[v][l2][c]>0.3))
		       {
	 ...
	 for (c=0;c<NBCTS;c++)
	   for (v=0;v<NVEG2;v++)
	     if ((meanmaxts[l2][c]>fillval)&&(tsmseuil[v][l2][c]<0))
	       {
		   for (rayon=5;(rayon<50)&&(tsmseuil[v][l2][c]<0);rayon+=5);
		     {
			 ...
			 for (c2=-rayon;c2<rayon;c2++)
			   {
			     ...
			     if (tsmseuil[v][lr][cr]>0)
			       {
				 if (tsmseuil[v][l2][c]<0) {tsmseuil[v][l2][c]=tsmseuil[v][lr][cr];nbvalseuil[v][l2][c]=0;}  else tsmseuil[v][l2][c]+=tsmseuil[v][lr][cr];
				 nbvalseuil[v][l2][c]++;
			       }
			   }
		       if (nbvalseuil[v][l2][c]>0) {tsmseuil[v][l2][c]/=nbvalseuil[v][l2][c];pointtraite++;}
		     }
	       }
	 printf ("%d points traités\n",pointtraite);
	 }*/
     ncount[0]=NVEG;
     ncount[1]=NBL;
     ncount[2]=NBCTS; 
