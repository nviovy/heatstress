...
float *anondvimin,*anondvivar,*anondviminavant,*anondviminapres,minanovi[NVEG][NBL][NBCTS],*yearmin,*anondvimean,anoprecmean[NBL][NBCTS],mints[NBL][NBCTS],difdmints[NBL][NBCTS],mintsta[NBL][NBCTS],*ndviap;
void main(int argc,char *argv[])
{
  ...
  int varanandviav,varanandviap,varanatstaav,varanatstaap,varanats,varanadifts,varanatsta,varanotsta,varanoprecav,varanoprecap,varanotstad,varanotstadap,varanotstadav,varantstaav,varantstaap,varndvim,varndvimin,varndvimax,varndviap;
...
ndviap=calloc (NBL*NBCTS*NVEG,sizeof(float));
 ...
 datatsta=calloc(9*NBL*NBCTS,sizeof(float));
    sprintf (nfic,"data/laipftglob_%d.nc",yearref);
ncidmean=ncopen(nfic,NC_NOWRITE);
 status=nc_inq_varid(ncidmean,"LAI",&nvarvim);
 printf ("ncidmean=%d nvar=%d\n",ncidmean,nvarvim);
 status=nc_inq_varid(ncidpcmean,"rain",&nvarpcm);
 if (blk==0)
   {
     printf("creat\n");
    sprintf (nfic,"data/analogtpftglobmodlai2_%d.nc",yearref);
   ...
   varyearmin= ncvardef(ncidparam,"yearmin",NC_FLOAT,3,ndim+1);
     ...
     varanadifts = ncvardef(ncidparam,"anadifts",NC_FLOAT,2,ndim+1);
     ...
     ncattput(ncidparam, varanadifts, "_FillValue",NC_FLOAT,1,&fillval);
     printf ("ncat 1\n");
 ...
 if (blk*NBL+NBL>NBLTS) nbl=NBLTS-blk*NBL; else nbl=NBL;
 	 for (l=0;l<nbl;l++)
	   for (c=0;c<NBCTS;c++)
	     for (v=0;v<NVEG;v++) minanovi[v][l][c]=1e10;
 printf ("nbl=%d\n",nbl);
...
status=ncvarget(ncidts,nvartsta,start,ncount,datatsta);
 ...
 for (year=PYEAR;year<=DYEAR;year++)
   {
     printf (" **************** %d **********************\n",year);
     sprintf (nfic,"data/laipftglob_%d.nc",year);
    ...
    ncvarget(ncidndvi,nvarveg,start,ncount,dataveg);
      for (t=60;t<244;t+=NBJNDVI)
       {

	 if (t%NBJNDVI==0)
	   {	   
	     printf ("%d\n",t);
	     start[0]=(t/NBJNDVI+1)%(365/NBJNDVI+1);
	     ncount[0]=1;
	     start[1]=0;
	     ncount[1]=NVEG;
	     start[2]=(blk*NBL);
 ...
 for (l=0;l<nbl;l++)
   for (c=0;c<NBCTS;c++) 
	 {
	   nbvmin[l][c]=nbvmints[l][c]=0;
	   mints[l][c]= mintsta[l][c]=0;
	   ...
	   for (year=PYEAR;year<=DYEAR;year++)
	     if (year!=yearref)
	       {
		   {
			 ...
			 if (((fabs(anondvi[(year-PYEAR)*NVEG*nbl*NBCTS+v*nbl*NBCTS+l*NBCTS+c])<minanovi[v][l][c]))&&(anondvi[(year-PYEAR)*NVEG*nbl*NBCTS+v*nbl*NBCTS+l*NBCTS+c]>fillval))
			   {
			     minanovi[v][l][c]=fabs(anondvi[(year-PYEAR)*NVEG*nbl*NBCTS+v*nbl*NBCTS+l*NBCTS+c]);
			     if (year>=2011)
			       {
				 mints[l][c]=anots[(year-PYEAR)*nbl*NBCTS+l*NBCTS+c];
				 difdmints[l][c]=datadmax[(year-PYEAR)*nbl*NBCTS+l*NBCTS+c]-datadmax[(yearref-PYEAR)*nbl*NBCTS+l*NBCTS+c];
 ...
 for (l=0;l<nbl;l++)
   for (c=0;c<NBCTS;c++) 
	 ...
	 {
	   for (y=0;y<(DYEAR-PYEAR+1);y++) anoprec[y*nbl*NBCTS+l*NBCTS+c]=anots[y*nbl*NBCTS+l*NBCTS+c]=anoprecavant[y*nbl*NBCTS+l*NBCTS+c]=anoprecapres[y*nbl*NBCTS+l*NBCTS+c]==fillval;
 ...
 ERRHAND(status);
   ...
   ERRHAND(status);
   status=nc_inq_varid(ncidparam,"anats",&varanadifts);
   ERRHAND(status);
     printf ("inq14\n");
 ...
 status=nc_inq_varid(ncidparam,"ndvimax",&varndvimax);
  ...
  if (blk==0)
     {
       sprintf (nfic,"lst/tsdfb_2012.nc");
  ...
  status=ncvarput(ncidparam,varanadifts,start,ncount,difdmints);
   printf ("put 4\n");
