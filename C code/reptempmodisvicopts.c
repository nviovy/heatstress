...
nyv=ncvardef(ncido,"TSmax_mean",NC_FLOAT,1,&nyii);
 ...
 for (year=PYEAR;year<=DYEAR;year++)
   {
     sprintf (nfic,"data/analogpftglob_%d.nc",year);
   ...
   printf ("get1\n");
	 ...
	 for (v=0;v<NVEG;v++) 
	   for (l2=0;l2<NBL;l2++)
		 ...
		 v2=v;
		 if ((danandviav[v][l2][c]>-3)&&(danandviav[v][l2][c]<=3)&&(danandviap[v][l2][c]>-3)&&(danandviap[v][l2][c]<=3))
		   {
		     // if ((v==0)) printf ("(%e %e %e)",danandviav[v][l][c],dndvim[l][c],danandviap[v][l][c]);	
		     ampli=dndvimax[v][l2][c]-dndvimin[v][l2][c];
		     ...
		     if ((diffvi<-0.1) && (danats[l2][c]>4)&&(dndviap[v][l2][c]>0.5))
		       //if ((danats[l][c]>4))
		       {
		     ...
		     if ((tsm>20)&&(tsm<65)&&(dndvim[v][l2][c]>0.4)&&( meanmaxts[l2][c]<45)&&(dndviap[v][l2][c]>0.3)&&(danats[l2][c]>0))
		       {
		     ...
		     if ((diffvi<-0.05)&&(tsm>43)&&(danats[l2][c]>4)&&(dndviap[v][l2][c]>0.2))
		       {
